...
const AttendanceLog = ({ onBack, onCheckIn, onSchedule, onReports, onLogout }) => {
    ...
    <div className="admin-page attendance-page">
      <aside className="admin-sidebar">
        ...
        <nav className="admin-nav">
          ...
          <button className="admin-nav-item selected"><span>[]</span>Attendance Log</button>
      ...
      <main className="admin-main">
        ...
        <div className="attendance-content">
          ...
          <section className="attendance-stat-grid">{[["56", "Checked In Today", "+12 vs Yesterday", "users", "blue"], ["8", "In Facility Now", "Live Status", "home", "green"], ["12", "Classes Today", "Scheduled", "calendar", "blue"], ["92%", "Attendance Rate", "vs Capacity", "pulse", "blue"]].map(([value, label, sub, icon, tone]) => <div className="attendance-stat" key={label}><span className={`attendance-icon ${tone}`}>{icon}</span><div><strong>{value}</strong><p>{label}</p><small>{sub}</small></div></div>)}</section>
